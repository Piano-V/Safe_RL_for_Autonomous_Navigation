...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Unconstrained Baseline SAC Agent with Static Penalty.")
    ...
    parser.add_argument("--beta", type=float, default=30.0, help="Static penalty weight for constraint breaches")
    parser.add_argument("--save-path", type=str, default="models/unconstrained_sac_actor.pth", help="Filepath to save the actor weights")
    return parser.parse_args()

def train_unconstrained(args):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Training Unconstrained (Static Penalty) SAC Agent on device: {device}")
    ...
    for episode in range(1, args.episodes + 1):
        ...
        while not done:
            ...
            with torch.no_grad():
                ...
                # Unconstrained SAC model with fixed static penalty beta
                backup = (rewards_t - args.beta * costs_t) + (1.0 - dones_t) * args.gamma * target_q
        ...
        if episode % 10 == 0:
            ...
            print(f"Episode: {episode:04d}/{args.episodes} | Avg Reward: {avg_reward:.1f} | Avg Cost: {avg_cost:.2f} | Penalty (Beta): {args.beta}")

    print(f"\nTraining Complete! Saving unconstrained actor weights to: {args.save_path}")
    os.makedirs(os.path.dirname(args.save_path), exist_ok=True)
    torch.save(actor.state_dict(), args.save_path)

if __name__ == "__main__":
    train_unconstrained(parse_args())
